...
bool TextureApp::create(const std::string &appName) {
    ...
    auto indexBufferCreator = rvkfw::RBufferObject::Creator(physicalDevice(), logicalDevice());
    indexBufferCreator.bufferInfo().usage = VK_BUFFER_USAGE_INDEX_BUFFER_BIT;
    mIndexBuffer = indexBufferCreator.create(indices.data(), indices.size() * sizeof(indices[0]));
    ...
    mImageBuffer = rvkfw::RImageBuffer::Creator(physicalDevice(), logicalDevice()).
                    create(mResourceDirectory + "/image.png");
...
void TextureApp::recordDrawCommands() {
...
float TextureApp::getRandom(float start, float end) const {
    auto rn = std::rand() % (1000+1);
    float t = rn / 1000.0f;
    return start * (1.0f-t) + end * t;
}

void TextureApp::updateTransformation(uint32_t imageIndex) {
...
void TextureApp::draw() const {
    mDrawHelper->draw(mCommandBuffer, [this](uint32_t imageIndex) {
        const_cast<TextureApp *>(this)->updateTransformation(imageIndex);
    });
}
